...
// --- Drag-to-reposition (tiles) ---
const applyGroupStep = (stepX, stepY, anchorId) => {
  if (!stepX && !stepY) return;
  setData(prev => {
    // If multi-select exists and includes the anchor, move all selected; else only anchor
    const group =
      typeof selectedIds !== 'undefined' && selectedIds.size && selectedIds.has(anchorId)
        ? selectedIds
        : new Set([anchorId]);
    return {
      ...prev,
      characters: prev.characters.map(c =>
        group.has(c.id)
          ? {
              ...c,
              gridX: Math.max(0, c.gridX + stepX),
              gridY: Math.max(0, c.gridY + stepY)
            }
          : c
      )
    };
...
const handleWindowMove = (e) => {
  ...
  const stepX = Math.trunc(d.accX / GRID_SIZE);
  const stepY = Math.trunc(d.accY / GRID_SIZE);
  if (stepX || stepY) {
    applyGroupStep(stepX, stepY, d.anchorId);
    d.accX -= stepX * GRID_SIZE;
    d.accY -= stepY * GRID_SIZE;
  }
};
...
const beginTileDrag = (e, anchorId) => {
  if (!isEditing) return;
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            data.characters.map((char) => {
              ...
              return React.createElement(CharacterTile, { // PROPS //
                {
                  key: char.id,
                  character: char,
                  isEditing: isEditing,
                  // keep your existing highlight/dim/sectColor if you already pass them:
                  // isHighlighted: ...,
                  // isDimmed: ...,
                  // sectColor: ...,
                  // onClick: ...,
                
                  // --- selection state (robust even if multi-select helpers don't exist) ---
                  isSelected: (typeof selectedIds !== 'undefined' && selectedIds.size
                    ? selectedIds.has(char.id)
                    : !!(selectedCharacter && selectedCharacter.id === char.id)),
                
                  onPrimarySelect: () => {
                    if (typeof setOnlySelected === 'function') {
                      setOnlySelected(char.id);
                    } else if (typeof setSelectedCharacter === 'function') {
                  ...
                  // --- start drag on mousedown (selection first, then drag) ---
                  onMouseDownStart: (e) => {
                    e.preventDefault();
                    if (e.shiftKey || e.metaKey || e.ctrlKey) {
                      if (typeof toggleSelected === 'function') {
                    ...
                    if (typeof beginTileDrag === 'function') {
                      beginTileDrag(e, char.id);
                    }
                  }
                });
            })
